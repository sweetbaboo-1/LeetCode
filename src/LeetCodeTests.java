...
public class LeetCodeTests {
    ...
    public void testTwoSum_pass() {
        RemoveDuplicatesFromSortedArray.TwoSum twoSum = new RemoveDuplicatesFromSortedArray.TwoSum();
        int[] answer = twoSum.twoSumTwoPointer(new int[]{2,5,5,11}, 10);
        Assertions.assertNotNull(answer);
//        Assertions.assertArrayEquals(answer, new int[]{1, 2});
    }
    ...
    public void AllOne_test() {
        ...
        for (int i = 0; i < cmds.length; i++) {
            String s = cmds[i];
            switch (s) {
                case "inc" -> allOne.inc(args[i]);
                case "dec" -> allOne.dec(args[i]);
                case "getMaxKey" -> System.out.println(allOne.getMaxKey());
                case "getMinKey" -> System.out.println(allOne.getMinKey());
            }
        }
    }
